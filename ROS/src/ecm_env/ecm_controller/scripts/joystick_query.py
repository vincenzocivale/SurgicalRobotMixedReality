...
class JoystickNode:
    def __init__(self, groups): # Il prametro "groups" deve essere una lista di stringhe, dove ogni stringa corrisponde al nome di un gruppo che si vuole controllare
        ...
        # TODO: Publisher per pubblicare la posizione target istante per istante
        self.target_point = rospy.Publisher('/target_point', Float64MultiArray, queue_size = 10)
        ...
        self.published_pose_once = False # Variabile che permette di controllare se la pubblicazione è già avvenuta, in modo tale che ad una singola pressione del pulsante "quadrato" corrisponda una singola pubblicazione dela posa target
    ...
    def update_arm_group_pose(self, data): # Funzione necessaria per aggiornare la posizione dell'arm in base all'input del joystick
        ...
        if data.buttons[3] == 1 and not self.published_pose_once: # Solamente quando viene premuto il tasto "quadrato"...
            ...
            rospy.loginfo(f"Execution! Target position for group number {self.current_group_index + 1}: ({self.x}, {self.y}, {self.z})")
            self.published_pose_once = True # In questo modo si evitano doppie pubblicazioni...quando viene premuto il tasto "quadrato" si effettua una singola pubblicazione
        elif data.buttons[3] == 0:
            # TODO: La riga di codice sottostante è stata aggiunta per pubblicare la posizione target istante per istante
            # Questo è stato fatto per realizzare un topic parallelo ad /arm_group che però non pubblichi solo quando viene premuto "quadrato"
            self.target_point.publish(self.arm_group_pose)
            self.published_pose_once = False
    ...
    def update_end_effector_group_pose(self, data): # Funzione necessaria per aggiornare la pose dell'end-effector in base all'input del joystick
        ...
        # NOTA: Si ricorda che l'end - effector viene controllato con la cinematica diretta; quindi, non si fornisce in input una posizione da raggiungere ma delle roto-traslazioni..
